...
use App\Enums\FrontendMessageType;
...
if (!function_exists('sendFlashMessage')) {
	function sendFlashMessage(string $message, FrontendMessageType $type = FrontendMessageType::DEFAULT): void {
		Session::put('message', [
			'type' => $type,
			'message' => $message,
		]);
...
if (!function_exists('sendFlashNotification')) {
	function sendFlashNotification(string $message, FrontendNotificationType $type = FrontendNotificationType::DEFAULT, ?string $description = null, ?int $duration = null): void {
